...
class Merger(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        if self.head > 1:
            return th.mean(self.softmax(self.weight) * x, dim=1, keepdim=False)
        else:
            return th.squeeze(x, dim=1)


class SPE_RNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        super(SPE_RNNAgent, self).__init__()
        self.args = args
    ...
    def forward(self, inputs, hidden_state):
        ...
        # embedding_enemies = embedding_enemies.mean(dim=1, keepdim=False)  # [bs * n_agents, n_head, rnn_hidden_dim]

        # TODO: check dimension
        embedding_enemies_org = self.pe_input_w_enemy(enemy_feats_t)
        embedding_enemies = embedding_enemies_org.mean(dim=-2, keepdim=False)  # [bs * n_agents, n_head, rnn_hidden_dim]
        ...
        # embedding_allies = embedding_allies.mean(dim=1, keepdim=False)  # [bs * n_agents, n_head, rnn_hidden_dim]

        embedding_allies_org = self.pe_input_w_ally(ally_feats_t)
        embedding_allies = embedding_allies_org.mean(dim=-2, keepdim=False)
